...
The app keeps the dataframe in memory and forwards a compact context (schema, a configurable leading-row sample that defaults to 20 rows and can be raised to 1,000, and `df.describe()`) to the model on each turn. Very wide samples are truncated to keep prompts manageable. For datasets where the full table matters, the assistant is prompted to say so rather than fabricating an answer.

> ⚠ **Picking the remote provider sends your data off this device** — the schema, configured row sample, and summary statistics will be transmitted to whatever API host you point it at. Use a local provider if your CSV contains anything sensitive.
...
The base URL is editable in Settings if you've moved a provider off its default port or want to point the remote provider at a different host (Together, Groq, etc.). All four runtimes expose an OpenAI-compatible `chat/completions` endpoint, so the app uses the `openai` SDK with the appropriate `base_url`.
