...
A [pixi](https://pixi.sh)-based [Streamlit](https://streamlit.io) app that lets you upload a CSV, browse it as a pandas table, and chat with the data through a local LLM.
...
2. Pick a provider — `llama.cpp`, `Ollama`, or `Docker Model Runner`.
...
| Provider | Default URL | Model discovery endpoint | OpenAI base URL |
|---|---|---|---|
...
The base URL is editable in the sidebar if you've moved a provider off its default port. All three runtimes expose an OpenAI-compatible `chat/completions` endpoint, so the app uses the `openai` SDK with the appropriate `base_url`.
