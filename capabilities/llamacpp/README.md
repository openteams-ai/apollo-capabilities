...
A [pixi](https://pixi.sh)-based workflow downloads a precompiled [llama.cpp](https://github.com/ggml-org/llama.cpp) binary, then launches a local inference server that auto-downloads the model and mmproj GGUFs from HuggingFace via llama.cpp's built-in `-hf` flag.
...
This automatically downloads the correct llama.cpp build for your platform on first run. The server then uses `-hf` to fetch the model and mmproj GGUFs into a local `models/` HF cache (`HF_HOME=models`), isolating downloads from any system-wide HuggingFace cache. Subsequent runs use the cached files. The server listens on llama.cpp's default port 8080.
...
To download just the llama.cpp binary without starting the server: `pixi run download-llamacpp` (or `pixi run -e gpu download-llamacpp`). Model GGUFs are fetched automatically on first `serve`.
