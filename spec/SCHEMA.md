...
- `[tool.capability]`
- `[tool.capability.<org-name>.<capability-key>]`
- `[tool.capability.<org-name>.<capability-key>.targets.<target>]`
...
## Why `spec-version` lives at `tool.capability`

`spec-version` describes the shape of the capability metadata block itself, not one individual capability definition. Keeping it at `[tool.capability]` avoids duplication and still leaves room for multiple capability definitions in a single manifest file.
...
[tool.capability.<org-name>.<capability-key>.targets.local]
task = "launch"
environment = "default" # optional, defaults to default

[tool.capability.<org-name>.<capability-key>.targets.hub]
task = "launch-hub"
environment = "default" # optional, defaults to default
